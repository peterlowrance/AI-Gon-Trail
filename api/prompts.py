...
def get_start_prompt(theme: str) -> Prompt:
    return {
        "prompt": f"""This is a game that is similar to "The Oregon Trail" but with a custom theme of "{theme}". Respond with a json object that shows the available items to purchase at the start of the game. Include a field "items" which is a map with keys of the item and value of the cost in the local currency. They will start with 100 to spend. There should be 20 items. The items should be things that can be stored in the group_base and that the group can use to overcome various challenges. "characters" is an array of 5 strings with the character's names. "group_base" is where the characters are and has (Health: 10/10). Include a field goal which is the characters goal which when accomplished will end the game. Include a field description which describes the group preparing to start out on their journey based on the theme. Each item, character, and the group_base can have modifiers for extraneous features, shown in parentheses with the item name, for example "gun (broken)", " or "shovel (strong, steel)". Half of the items have modifiers. All the fields should match the theme.
Example:
{{
	"items": {{"shovel (steel)": 12, "hunting knife": 15, "rations (stale)": 3}},
	"characters": ["Bob (healthy, cartographer)", ...],
	"group_base": "Wagon (Health 10/10)",
	"goal": "Travel to the west",
	"description": "The group sets out..."
}}""",
        "temperature": 0.7,
        "response_type": "json",
        "validation_schema": {"items": {"*": int}, "characters": [str], "group_base": str, "goal": str, "description": str}
    }
...
The state of the game before encountering the situation is items: "{', '.join(state.items)}"; characters: "{', '.join(state.characters)}"; and group base: {state.group_base}.
...
def get_scenario_list_prompt(theme: str, goal: str) -> Prompt:
    return {
        "prompt": f"""This is a game that is similar to The Oregon Trail but with a custom theme of "{theme}" with the goal of "{goal}". Your job is to generate 8 short situations of high difficulty for the player to encounter on their attempt to {goal} that they will try to overcome.
The situations should be related to the theme. The situations should involve challenges that need to be overcome in order to pursue the goal. None of the situations should mention the players supplies or the passage of time as those are handled elsewhere. 
...
For example, if the player chooses to cook a meal to pass through a treacherous mountain pass, the outcome should include both the positive effects of the meal (increased energy and morale) and the negative consequences of not directly addressing the challenges of the mountain pass (possible injuries, group_base damage, or lost items).

Respond with a brief description of the outcome and provide updated items, characters, and group_base based on the scenario, the action, and the outcome. The outcome should conclude the scenario so the next scenario can be faced. If an item was used, remove it from the list. If a character died, remove them from the list, if the health of the group_base changed, change it in the response. If a change happened to an item or character you may update them by adding or changing modifiers in parenthesis, example "(hunter) John" becomes "(sick, hunter) John". The scenario is {state.get_difficulty()}, but extremely negative outcomes should be rare. Example format:
{{"outcome":"description", "items":["(damaged) {example_item}", ...], "characters":["(injured, farmer) {state.characters[0]}", ...], "group_base": "{state.group_base}"}}

Respond with only the json object""",
        "temperature": .8,
        "response_type": "json",
        "validation_schema": {"outcome": str, "items": [str], "characters": [str], "group_base": str}
    }
...
Scenario: "{scenario}"
The player action is "{player_action}"
Consider the risk factor and effectiveness of the player's action when generating the outcome. If the action seems insufficient or unlikely to prevent damage, introduce negative consequences that logically stem from the player's choice. Make the outcome more realistic by considering the actual effectiveness of the chosen action in addressing the core challenges of the scenario, and include any negative consequences that may arise due to the chosen action. Negative consequences can lose items, kill characters, lower group_base health, and change items or characters to add negative modifiers. For example, if the player chooses to cook a meal to pass through a treacherous mountain pass, the outcome should include both the positive effects of the meal (increased energy and morale) and the negative consequences of not directly addressing the challenges of the mountain pass (injuries, group_base damage, and lost items). If the group_base is unusable after the outcome, set its health to 0.
Respond with a json object with the following fields to describe the status after the outcome is completed: "outcome" is a story-like description of the outcome, "items_lost" is the list of items that were lost, "items_gained" is the list of items gained, "items_changed" is a map of items to the item with changed modifiers in parenthesis, "characters_lost" is the characters that have died or were lost, "characters_gained" are any new characters, "characters_changed" is a map of character to the character with changed modifiers in parenthesis, and "group_base" is the group_base with updated health. Fill these out based on the action and the outcome. The outcome should conclude the scenario so the next scenario can be faced. If a change happened to an item or character you must include them in "items_changed" or "characters_changed" with a full list of their modifiers in parenthesis, including their original modifiers; example "characters_changed": {{"John (hunter)": "John (sick, hunter)"}}. Example: "items_changed": {{"blanket": "blanket (wet)"}}. Only include things in "items_changed" or "characters_changed" when the modifiers change. "quote" is a {quote_type} thing that a character said about the outcome. The scenario is {state.get_difficulty()} difficulty but extremely negative outcomes should be rare.
Example format:
{{"outcome": "description", "items_lost": ["{another_example_item}"], "items_gained": [], "items_changed": {{"{example_item}": "{example_item_changed}", ...}}, "characters_lost": ["{state.characters[0]}"], "characters_gained": [], "characters_changed": {{"{example_character}": "{example_character_changed}", ...}}, "group_base": "{state.group_base}", "quote": "{Item(random.choice(state.characters)).key}: In hindsight..."}}
Respond with only the json object""",
        "temperature": .8,
        "response_type": "json",
        "validation_schema": {"outcome": str, "items_lost": [str], "items_gained": [str], "items_changed": {'*': str}, "characters_lost": [str], "characters_gained": [str], "characters_changed": {'*': str}, "group_base": str, "quote": str}
    }

def get_game_end_prompt(prev_outcome: str, state: GameState) -> Prompt:
    goal = 'successfully accomplished their goal' if state.game_over == 'WIN' else 'failed trying to accomplish their goal'
    if state.game_over == 'WIN':
        end_reason = f'accomplishing their goal {state.goal}'
    else:
        end_reason = 'losing because all the characters died' if len(state.characters) == 0 else "losing because the group_base didn't survive"
    ...
    return {
        "prompt": f"""The theme is {state.theme}. The characters have {goal} {state.goal} after facing several challenges. Here is a description of their last challenge: "{prev_outcome}".
The journey began with characters "{', '.join(state.original_characters)}"{end_characters}.
Their group_base ended up as {state.group_base}.
