...
def get_validate_action_prompt(scenario: str, items: list[str], characters: list[str], action: str) -> Prompt:
    return {
        "prompt": f"""Your job is to determine whether or not a payers action is valid, feasible, or possible. If the player action involves an object not in the Scenario, Available items, or Characters it is not valid.
...
Is this player action valid? Respond in the format {{"valid": boolean, "explanation": string}}""",
        "temperature": 0,
        "response_type": "json",
